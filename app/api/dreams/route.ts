...
export async function GET() {
  ...
  const item = await prisma.dreamFood.findFirst({
...
export async function PUT(request: Request) {
  ...
  const existing = await prisma.dreamFood.findFirst({
    where: { userId: session.user.id },
  })

  let item
  if (existing) {
    item = await prisma.dreamFood.update({
      where: { id: existing.id },
      data: { content },
    })
  } else {
    item = await prisma.dreamFood.create({
